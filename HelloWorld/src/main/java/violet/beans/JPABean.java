...
public class JPABean {
	...
	public Map<String, Game> getGameMap(String platform)
	{
		EntityManager em = getEMF().createEntityManager();
		try
		{
			TypedQuery<Game> g = em.createQuery("SELECT g FROM Game g WHERE g.platform=:platform", Game.class);
			List<Game> resultList = g.getResultList();
			Map<String, Game> resultMap = new HashMap<String, Game>(resultList.size());
			for(Game game : resultList)
			{
				resultMap.put(game.getId(), game);
			}
			return resultMap;
		}
	...
	public boolean addGames(List<Game> list)
	{
		for(Game game : list)
		{
			EntityManager em = getEMF().createEntityManager();
